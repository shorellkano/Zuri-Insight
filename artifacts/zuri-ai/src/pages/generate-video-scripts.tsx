...
export default function GenerateVideoScripts() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-5" data-testid="generate-video-scripts-page">
      <div className="flex items-center gap-3">
        <div className="h-10 w-10 rounded-xl bg-purple-100 flex items-center justify-center shrink-0">
          <Video className="h-5 w-5 text-purple-700" />
        </div>
        <div>
          <h1 className="text-xl font-bold text-foreground">Video Script Generator</h1>
          <p className="text-muted-foreground text-sm">Hook, body, and CTA scripts for TikTok, Reels, and YouTube Shorts.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-6">
        <div className="bg-card border border-border rounded-2xl p-6">
          ...
          <GeneratorForm type="video-scripts" onGenerate={onGenerate} isPending={generate.isPending} />
        </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 min-h-[500px]">
          ...
          {output ? (
            <ContentOutput variations={output.variations} type="video-scripts" brandId={output.brandId} onRegenerate={onRegenerate} isRegenerating={generate.isPending} />
          ) : (
            <EmptyOutputState type="video-scripts" />
          )}
        </div>
      </div>
    </div>
