...
import { Loader2, CalendarDays, Check, Trash2, AlertCircle, CheckCircle2 } from "lucide-react";
...
const PLATFORM_ICONS: Record<string, string> = { instagram: "IG", facebook: "FB", tiktok: "TT", linkedin: "LI", youtube: "YT" };
...
export default function BulkPlan() {
  ...
  const [step, setStep] = useState<"setup" | "suggestion" | "generating">("setup");
  ...
  const [generatedItems, setGeneratedItems] = useState<Record<string, boolean>>({});
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          Zuri builds your entire content calendar, covers all key dates, and schedules everything - you just approve.
        </p>
      </div>
      ...
      {step === "suggestion" && items.length > 0 && (
        <div className="space-y-5">
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            ...
            <p className="text-xs text-amber-700 mt-0.5">Zuri always suggests before it produces. You are in control - adjust dates, delete slots, or approve as-is.</p>
          </div>
              ...
              {items.map(item => (
                ...
                >
                  ...
                  <div className="px-3 py-3">
                    <span className="px-2 py-0.5 bg-muted rounded text-xs font-medium capitalize">{item.platform}</span>
                  </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button
              className="flex-1 sm:flex-none px-6 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-semibold hover:bg-primary/90 transition-colors"
              onClick={() => toast({ title: "Content generation coming soon - approve your plan first and Zuri will generate everything in sequence." })}
            >
