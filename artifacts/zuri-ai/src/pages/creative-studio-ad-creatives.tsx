...
export default function CreativeStudioAdCreatives() {
  ...
  async function generate() {
    if (!activeBrandId) {
      toast({ description: "Select a brand first", variant: "destructive" });
      return;
    }
    setLoading(true);
    setHtml(null);
    ...
    try {
      const r = await fetch(API("/generate/ad-creative"), {
        ...
        body: JSON.stringify({
          ...
          showBrandName,
          customPhotoDataUrl: customPhotoDataUrl || undefined,
        }),
      });
  ...
  async function downloadPng() {
    ...
    try {
      ...
      const canvas = await html2canvas(container, {
        width: dims.w, height: dims.h, scale: 1, useCORS: true, backgroundColor: null,
      });
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-medium text-foreground">Ad Creatives</span>
        </div>
      </div>
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Generate on-brand ad visuals for Meta, TikTok, Google Display and Snapchat — ready to upload.
        </p>
      </div>
        ...
        <div className="space-y-5 bg-card border border-border rounded-2xl p-6">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {PLATFORMS.map(({ id, label, Icon, color }) => (
                ...
                >
                  <span className={cn("w-6 h-6 rounded-md flex items-center justify-center shrink-0", color)}>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1.5">
              {currentPlatform.formats.map(f => (
                <button
                  key={f.id}
                  onClick={() => { setAdFormat(f.id); setHtml(null); setGenerated(null); }}
                  className={cn(
                    "w-full flex items-center justify-between px-4 py-2.5 rounded-xl border-2 text-sm transition-all text-left",
                    adFormat === f.id
                      ? "border-primary bg-primary/8 text-primary"
                      : "border-border hover:border-primary/30 text-foreground",
                  )}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              What are you promoting? <span className="normal-case font-normal">(optional)</span>
            </label>
            <textarea
              value={offerText}
              onChange={e => setOfferText(e.target.value)}
              placeholder="e.g. 30% off our handmade skincare range this weekend only — free delivery on orders over ₦5,000"
              rows={3}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              Headline <span className="normal-case font-normal">(optional — Zuri will write one)</span>
            </label>
            <input
              type="text"
              value={headline}
              onChange={e => setHeadline(e.target.value)}
              placeholder="e.g. Your Skin Deserves Better"
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="e.g. Handcrafted with natural Nigerian ingredients"
              className="w-full px-3.5 py-2.5 rounded-lg border border-border bg-background text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
            />
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              CTA button text <span className="normal-case font-normal">(optional)</span>
            </label>
            <input
              type="text"
              value={ctaText}
              onChange={e => setCtaText(e.target.value)}
              placeholder="e.g. Shop Now, Order Today, Get 30% Off"
              className="w-full px-3.5 py-2.5 rounded-lg border border-border bg-background text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
            />
          </div>
          ...
          <button
            ...
            className={cn(
              ...
              loading || !activeBrandId
                ? "bg-muted text-muted-foreground cursor-not-allowed"
                : "bg-primary hover:bg-primary/90 text-white",
            )}
          >
            {loading
              ? <><Loader2 className="h-4 w-4 animate-spin" /> Generating ad...</>
              : <><Sparkles className="h-4 w-4" /> Generate Ad Creative</>}
          </button>

          {!activeBrandId && (
            <p className="text-xs text-center text-muted-foreground">Select a brand from the top bar to get started</p>
          )}
        </div>
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-muted-foreground max-w-xs">
                      Configure your ad above and click Generate. Your brand colors will be used automatically.
                    </p>
                  </>
                )}
              </div>
            ...
            <div className="bg-card border border-border rounded-xl p-4 space-y-2">
              ...
              <div className="space-y-1.5">
                <div className="flex gap-2">
                  <span className="text-xs text-muted-foreground w-16 shrink-0">Headline</span>
                  <span className="text-xs font-semibold text-foreground">{generated.headline}</span>
                </div>
                <div className="flex gap-2">
                  <span className="text-xs text-muted-foreground w-16 shrink-0">Tagline</span>
                  <span className="text-xs text-foreground">{generated.tagline}</span>
                </div>
                <div className="flex gap-2">
                  <span className="text-xs text-muted-foreground w-16 shrink-0">CTA</span>
                  <span className="text-xs font-semibold text-primary">{generated.cta}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {html && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                onClick={generate}
                disabled={loading}
                className="flex items-center justify-center gap-2 py-2.5 bg-primary/10 text-primary border border-primary/20 rounded-xl text-sm font-medium hover:bg-primary/20 transition-colors"
              >
