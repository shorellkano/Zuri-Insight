...
const TONES = ["Friendly", "Urgent", "Promotional", "Informational", "Conversational"];
...
export default function GenerateWhatsapp() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6" data-testid="generate-whatsapp-page">
      <div className="flex items-center gap-3">
        <div className="h-10 w-10 rounded-xl bg-green-100 flex items-center justify-center">
          <MessageCircle className="h-5 w-5 text-green-700" />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-foreground">WhatsApp Message Generator</h1>
          <p className="text-muted-foreground text-sm">Create conversational messages for WhatsApp marketing and broadcast lists.</p>
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          {output ? (
            <ContentOutput variations={output.variations} type="whatsapp" />
          ) : (
            <div className="flex flex-col items-center justify-center h-full py-16 text-center bg-muted/30 border border-dashed border-border rounded-2xl">
              ...
              <p className="text-sm text-muted-foreground">Generated WhatsApp messages will appear here.</p>
            </div>
          )}
        </div>
      </div>
